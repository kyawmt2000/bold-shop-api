...
def upload_file_to_firebase(file_obj, filename_prefix="outfits/"):
    ...
    # 2. 初始化客户端 (使用硬编码路径，绕过 Render 环境变量加载 Bug)
    try:
        # ⭐ 核心修复：直接使用文件名，确保 Client 找到密钥文件
        cred_path = "service-account-key.json" 
        
        # 显式使用密钥文件路径初始化客户端
        # 这确保了 Client 知道密钥文件的确切位置，从而解决 500 认证失败的问题。
        storage_client = storage.Client.from_service_account_json(cred_path)
        
    except Exception as e:
        # 如果初始化失败，说明密钥文件路径或内容有问题
        logging.error(f"Firebase Client initialization failed: {e}")
        # 抛出更清晰的错误
        raise Exception(f"Firebase 密钥文件加载失败。请检查 service-account-key.json。")

    # 3. 准备上传
    bucket = storage_client.bucket(bucket_name)

    # 4. 生成唯一的路径和文件名 (防止文件重名)
    ...
    # 7. 返回公开访问 URL
    # 这里我们使用 blob.public_url，它会返回一个更干净的 Google Cloud Storage URL
...
# app.py 约 30-40 行，在 db = SQLAlchemy(app) 之后
# -------------------- Firebase Storage Logic --------------------
def upload_file_to_firebase(file_obj, filename_prefix="outfits/"):
    """将文件上传到 Firebase Storage (基于 Google Cloud Storage)"""
    # 1. 检查配置
    ...
    # 5. 返回公开访问 URL (这是 Firebase 的标准格式)
    return f"https://firebasestorage.googleapis.com/v0/b/{bucket_name}/o/{unique_filename.replace('/', '%2F')}?alt=media"
